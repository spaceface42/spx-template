...
import { ContainerDimensions, 
         FloatingImageOptions } from '../../types/features.js';
...
export class FloatingImage {
    ...
    private vx: number;
    private vy: number;
    private options: FloatingImageOptions;

    constructor(element: HTMLElement, dims: ContainerDimensions, options: FloatingImageOptions = {}) {
    ...
    update(multiplier: number, dims: ContainerDimensions, applyPosition = true): boolean {
        if (!this.element) return false;
    ...
    resetPosition(dims: ContainerDimensions) {
        this.x = Math.random() * (dims.width - this.size.width);
        this.y = Math.random() * (dims.height - this.size.height);
        this.updatePosition();
    }
    ...
    clampPosition(dims: ContainerDimensions) {
        this.x = clamp(this.x, 0, dims.width - this.size.width);
        this.y = clamp(this.y, 0, dims.height - this.size.height);
    }
